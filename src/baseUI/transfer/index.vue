...
export default {
  ...
  methods: {
    ...
    async handleComfirm() {
      ...
      if (addData.length !== 0) {
        await this.addMemberFn({ ...this.info, memberSets: addData });
      }
      if (removeData.length !== 0) {
        await this.removeMemberFn({ ...this.info, memberSets: removeData });
      }
    ...
    getUserListFn() {
      ...
      this.getMemberFn(data).then((res) => {
        if (res.userInfoVOList) {
